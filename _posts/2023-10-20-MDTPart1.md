   ...
   Try
{
   Write-Output 'Adding Windows feature NET-Framework-Core'
	...
	Invoke-WebRequest 'https://raw.githubusercontent.com/keithga/DeployShared/master/Templates/Distribution/Scripts/Extras/Install-Chocolatey.ps1' -Outfile 'c:\temp\Install-Chocolatey.ps1'
  	...
  	Write-Output 'Creating MDT Service Account' 
	New-LocalUser "SVC_MDT" -Password "7tILC281eKeU" -FullName "MDT Service Account" -Description "MDT Service Account voor Windows Deployment"  
   ...
   Write-Output 'Setting up MDT'
    ...
    new-item -path "DS001:\Selection Profiles" -enable "True" -Name "WIN10X64" -Comments "" -Definition "<SelectionProfile><Include path=`"Packages\WIN11X64`" /></SelectionProfile>" -ReadOnly "False" -Verbose
	...
	Add-Content $MDTDeploymentShareFolder\control\Bootstrap.ini "UserPassword=7tILC281eKeU"
    ...
    Import-WdsBootImage -NewImageName 'Windows Deployment (x64)' -path $(Join-Path $MDTDeploymentShareFolder 'Boot\LiteTouchPE_x64.wim')
    Import-WdsBootImage -NewImageName 'Windows Deployment (x86)' -path $(Join-Path $MDTDeploymentShareFolder 'Boot\LiteTouchPE_x86.wim')
